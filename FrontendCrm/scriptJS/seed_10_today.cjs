...
async function createExcel() {
    ...
    const fileName = path.join(__dirname, `../Excel/DuLieuTest_10_${fileNameTime}.xlsx`);

    const nameTime = `${h}h${m}p${day}${month}${year}`;
    const prefixTime = `${h}${m}_${day}${month}${year}`;

    console.log(`Đang tạo 10 dòng dữ liệu test chống trùng lặp...`);
    ...
    console.log(`Đã tạo thành công file Excel 10 dòng tại: ../Excel/DuLieuTest_10_${fileNameTime}.xlsx`);
}
